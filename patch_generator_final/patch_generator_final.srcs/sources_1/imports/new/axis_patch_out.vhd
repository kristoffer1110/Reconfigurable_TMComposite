...
--
-- Serialises one patch from patches_v5 into a stream of 34 x 32-bit AXI-Stream
-- words and presents them on m_axis.
--
-- patch_last_in is NOT used here.  tlast is asserted with the final word
-- (word_cntr = NUM_WORDS-1) of EVERY patch, so each patch is an independent
-- AXI-Stream transaction.  The testbench can therefore receive and verify
-- patches individually without needing to know the total patch count.
--
-- Timing:
--   Cycle 0 : patch_valid_in='1', patch_ready_out='1'  -> data captured, busy='1'
--   Cycle 1 : tvalid='1', tdata=word[0], tlast='0'
--   ...
--   Cycle 34: tvalid='1', tdata=word[33], tlast='1'
--             (if tready='1' throughout; stalls otherwise)
--   Cycle 35: busy='0', patch_ready_out='1' again
--
-- Payload word layout (MSB-first serialisation of the flat payload):
--   word 0  : payload[1087:1056]   (top of y_pos)
--   word 33 : payload[31:0]        (bottom padding / last channel bits)
----------------------------------------------------------------------------------
...
begin
    ...
    m_axis_tvalid <= busy;
    m_axis_tlast  <= '1' when busy = '1' and word_cntr = NUM_WORDS - 1
                         else '0';
